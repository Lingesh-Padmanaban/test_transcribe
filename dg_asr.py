...
class TranscriptionService():
    def __init__(self):
        ...
        self.final_speech_indicate = None
        
    def initialize(self):
        self.dg_connection.on(LiveTranscriptionEvents.Open, self.on_open)
        self.dg_connection.on(LiveTranscriptionEvents.Transcript, self.on_message)

    def on_open(self, it, open, **kwargs):
        #print(f"Connection Open")
        self.inital_message_say = 1
    ...
    def on_message(self, it, result, **kwargs):
        ...
        if result.is_final:
            ...
            if result.speech_final:
                ...
                self.speech_final = last_utter
                self.final_speech_indicate = 0
                is_finals = []
            else:
                self.speech_final = None
        else:
    ...
    def on_utterance_end(self, it, utterance_end, **kwargs):
        try:
            global is_finals
            if len(is_finals) > 0:
                utterance = " ".join(is_finals)
                self.speech_final = utterance
                is_finals = []
            else:
                self.speech_final = last_utter
                self.vad_indicate = 0
